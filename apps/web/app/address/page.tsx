...
const Address = () => {
  const [addresses, setAddresses] = useState<Address | null>(null); // ✅ Single address object or null
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setAddresses(editing); // ✅ Update single address
      setEditing(null);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const handleDelete = async (id: string) => {
    ...
    setAddresses(null); // ✅ Set to null after delete
  };
    ...
    return (
      <div className="min-h-[90vh] flex items-center justify-center bg-[#F9FAFB] p-4">
        <div className="text-center py-12 rounded-2xl px-8 max-w-md">
          ...
          <h3 className="mt-6 text-xl font-bold text-gray-900">No address found</h3>
    ...
    <div className="min-h-[90vh] bg-[#F9FAFB] p-4 md:p-12 text-gray-500">
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10">
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight text-gray-600">Shipping Address</h1>
            <p className="text-slate-500 mt-1">Manage your shipping address.</p>
          </div>
        </div>
          ...
          <div className="max-w-2xl">
            ...
            >
              ...
              <div className="flex justify-between items-start mb-4">
                <h3 className="font-bold text-lg text-gray-500">{addresses.name}</h3>
                <div className="flex gap-2">
                  <button 
                    onClick={() => setEditing(addresses)} 
                    className="p-2 text-slate-400 hover:text-[#009FFF] hover:bg-indigo-50 rounded-lg transition-colors"
                  >
              ...
              <p className="text-sm text-slate-600 leading-relaxed space-y-1">
                <span className="block font-medium text-slate-800">
                  {addresses.street} {addresses.apartment && `• ${addresses.apartment}`}
                </span>
                <span className="block">{addresses.city}, {addresses.state} {addresses.zip}</span>
                <span className="block text-xs font-bold text-slate-400 uppercase tracking-tighter mt-2">
                  {addresses.country}
                </span>
              </p>
      ...
      <div className="p-6">
        <div className="grid grid-cols-2 gap-x-3 gap-y-3">
          {[
            ...
            { label: "Apartment", key: "apartment", span: "col-span-2" },
            { label: "City", key: "city", span: "col-span-1" },
            { label: "State", key: "state", span: "col-span-1" },
            { label: "Zip", key: "zip", span: "col-span-1" },
            { label: "Country", key: "country", span: "col-span-1" },
            { label: "Phone", key: "phone", span: "col-span-2" },
          ].map((field) => (
            <div key={field.key} className={field.span}>
              <label className="block text-[9px] font-semibold text-slate-500 uppercase tracking-wide mb-1">
                {field.label}
              </label>
              <input
                value={(editing as any)[field.key] || ""}
                onChange={(e) => setEditing({ ...editing, [field.key]: e.target.value })}
                className="w-full h-9 px-3 text-sm bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-[#009FFF] focus:bg-white focus:border-transparent outline-none transition-all"
              />
            </div>
          ))}
        </div>
        ...
        <div className="flex justify-end gap-3 mt-6">
          <button 
            ...
            className="h-9 px-5 text-sm text-slate-500 font-semibold hover:text-slate-700 transition-colors"
          >
            Cancel
          </button>
          <button 
            onClick={handleSave} 
            disabled={saving} 
            className="h-9 px-6 text-sm bg-[#009FFF] hover:bg-indigo-700 text-white font-semibold rounded-lg transition-all shadow disabled:opacity-50"
          >
            {saving ? "Saving..." : "Save"}
          </button>
        </div>
